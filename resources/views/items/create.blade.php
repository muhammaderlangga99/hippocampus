    ...
    <form method="POST" action="{{ route('items.store') }}" class="mt-10 lg:w-10/12 m-auto" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <input type="text" id="item"
                ...
                placeholder="Swimsuit" name="name" value="{{ old('name') }}" required>
            @error('name')
                <div class="text-xs text-red-600 font-medium pb-3">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="number" id="price"
                ...
                placeholder="100000" name="price" value="{{ old('price') }}" required>
            @error('price')
                <div class="text-xs text-red-600 font-medium pb-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-6">
            ...
            <input type="number" id="discount"
                ...
                placeholder="25" required name="discount" value="{{ old('discount') }}">
            @error('discount')
                <div class="text-xs text-red-600 font-medium pb-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-6">
            ...
            <input type="text" id="link"
                ...
                placeholder="https://www.tokopedia.com/******/***" required name="link" value="{{ old('link') }}">
            @error('link')
                <div class="text-xs text-red-600 font-medium pb-3">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-8">
            ...
            <textarea name="content" class="border-2 rounded-2xl @error('content') border-red-600 ring-red-600 @enderror">{{ old('content') }}</textarea>
